/**
 * Empire Hussein — Catalogue Civil
 * Charge data/catalogue-civil.json et affiche les ressources
...
const JSON_URL         = '../data/catalogue-civil.json';
const CART_STORAGE_KEY = 'empire-hussein:catalogue-civil:cart';
const CHECKOUT_CD_KEY  = 'empire-hussein:catalogue-civil:checkout-cd';
const CHECKOUT_CD_MS   = 60000;
const SOLARI_EMOJI     = '<:solari:1493385994479599686>';

/* Webhook civil — à configurer */
const _DT_CIVIL = null;
const _dest = () => _DT_CIVIL;
    ...
    return allItems.filter(item => {
        ...
        if (currentSearch) {
            const haystack = [
                ...
                CIVIL_LABELS[item.categorie] || item.categorie
...
function populateItemDropdown() {
    ...
    withPrice.forEach(i => {
        const g = CIVIL_LABELS[i.categorie] || i.categorie || 'AUTRE';
...
function itemTemplate(item) {
    ...
    const metaRows = [];
    if (item.fabricant)     metaRows.push(['ORIGINE', item.fabricant]);
    if (item.inspiration)   metaRows.push(['REFERENCE', item.inspiration]);
    ...
    return `
        <article class="cat-item" data-id="${esc(item.id)}">
            <div class="${imgClass}">
                ${imgBlock}
                ${niv && item.niveau ? `<span class="cat-item-niv">NIV ${niv}</span>` : ''}
            </div>
            <div class="cat-item-body">
                ...
                <div class="cat-item-subtype">${esc(item.soustype || CIVIL_LABELS[item.categorie] || '')}</div>
...
function buildOrderMessage() {
    ...
    for (const [cat, items] of groups) {
        const catLabel = CIVIL_LABELS[cat] || cat.toUpperCase();
...
async function validateOrder() {
    ...
    const dest = _dest();
    if (!dest) {
        showToast('Webhook non configuré — contactez un administrateur.');
        return;
    }
