(function () {
    ...
    function addPanZoom(svg, VW, VH) {
        ...
        svg.addEventListener('touchmove', function (e) {
            ...
            if (t.length === 1 && touches.length === 1) {
                ...
                if (pd > 0) { const [cx, cy] = clientToSVG((t[0].clientX + t[1].clientX)/2, (t[0].clientY + t[1].clientY)/2); zoomAt(cx, cy, pd / nd); }
            }
            touches = t;
        }, { passive: false });
    ...
    function buildMap(data, empireIds) {
        ...
        for (const f of data.features) { if (!empireIds.has((f.properties || {}).region_id)) addRegion(f, false); }
        for (const f of data.features) { if ( empireIds.has((f.properties || {}).region_id)) addRegion(f, true);  }
